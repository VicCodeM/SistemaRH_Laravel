    ...
    try:
        ...
        for line in lines:
            if line.startswith('MAIL_ENCRYPTION='):
                updated_lines.append('MAIL_ENCRYPTION=\n')
            else:
        ...
        # 2. Run clear config and check config
        cmd_clear = "cd /home/victor/sistemarh && php artisan config:clear && php artisan config:cache"
...
echo "CONFIG: " . var_export(config('mail.mailers.smtp'), true) . "\\n";
...
try {
    Mail::raw('Este es un correo de prueba automatizado para verificar que la configuración de Resend está funcionando correctamente.', function($message) {
        $message->to('luisyoumi@gmail.com')
                ->subject('Prueba de correo exitosa - SistemaRH (Fixed)');
    });
    echo "CORREO_ENVIADO\\n";
} catch (\\Throwable $e) {

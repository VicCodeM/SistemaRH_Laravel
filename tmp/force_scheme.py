...
def force_scheme_and_test():
    ...
    try:
        ...
        # 1. Append MAIL_SCHEME=null to .env using echo (faster)
        cmd_append = f"echo 'MAIL_SCHEME=null' >> {env_path}"
        ssh.exec_command(cmd_append)
...
try {
    Mail::raw('Este es un correo de prueba automatizado para verificar que la configuración de Resend está funcionando correctamente.', function($message) {
        $message->to('luisyoumi@gmail.com')
                ->subject('Prueba de correo exitosa - SistemaRH (Final)');
    });
    echo "CORREO_ENVIADO\\n";
} catch (\\Throwable $e) {

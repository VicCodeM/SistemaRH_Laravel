<x-app-layout>
    ...
    @if(!empty($alertas))
        <div style="display:grid; gap:10px; margin-bottom:20px;">
            @foreach($alertas as $alerta)
                <div class="alert alert-{{ $alerta['tipo'] }} fade-in" style="display:flex; justify-content:space-between; align-items:center;">
    ...
    <div class="metrics-grid fade-in">
        <div class="metric-card" style="{{ $stats['empresas_pendientes'] > 0 ? 'border-color: rgba(245,158,11,.4);' : '' }}">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;">
                <span class="metric-change" style="color:var(--text-muted);">{{ $stats['empresas_activas'] }} activas</span>
        ...
        <div class="metric-card">
            <div class="metric-top">
                <span class="metric-label">Personal interno</span>
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;">
                <span class="metric-change" style="color:var(--text-muted);">Internos activos en el sistema</span>
        ...
        <div class="metric-card" style="{{ $stats['tareas_activas'] > 0 ? 'border-color: rgba(14,165,233,.4);' : '' }}">
            <div class="metric-top">
                <span class="metric-label">Tareas de servicio</span>
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;">
                <span class="metric-change" style="color:var(--text-muted);">Activas y en proceso</span>
    ...
    <h2 style="font-size:1.1rem;font-weight:700;margin:28px 0 12px;color:var(--text-primary);">🔐 Aprobaciones de acceso pendientes</h2>
    ...
    <h2 style="font-size:1.1rem;font-weight:700;margin:28px 0 12px;color:var(--text-primary);">📋 Operaciones de servicio recientes</h2>
    <div class="card fade-in">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;">
                ...
                <table>
                    ...
                    <tbody>
                        @foreach($solicitudes_recientes as $s)
                            <tr>
                                <td>{{ $s->empresa?->nombre_empresa ?? '—' }}</td>
                ...
                <table>
                    ...
                    <tbody>
                        @foreach($tareas_recientes as $tarea)
                            @php $nivelServicio = $tarea->servicio?->nivel_jerarquico; @endphp
                            <tr>
                                <td>
                                ...
                                <td>{{ \App\Models\ServicioAsignado::asignableTipoLabel($tarea->asignable_type) }}: {{ $tarea->asignableNombre() }}</td>
                                <td>{{ $tarea->asignadoA?->name ?? 'Sin asignar' }}</td>

...
<nav class="sidebar">
    <div class="sidebar-brand">
        <a href="{{ route('dashboard') }}">
            <h2>Sistema<span>RH</span></h2>
            <span class="sidebar-subtitle">Panel {{ ucfirst($rol ?? '') }}</span>
        </a>
    </div>

    @if($rol === 'admin')
        <form method="GET" action="{{ route('admin.buscar') }}" style="padding:12px 16px; border-bottom:1px solid rgba(255,255,255,0.06);">
            <div style="display:flex; gap:6px; align-items:center; background:rgba(255,255,255,0.08); border-radius:8px; padding:6px 10px;">
                ...
                <input type="text" name="q" value="{{ request('q') }}" placeholder="Buscar..." style="background:transparent; border:none; color:#e2e8f0; font-size:13px; width:100%; outline:none;">
            </div>
        </form>
    @endif
    <div class="sidebar-nav">
        ...
        @if($rol === 'admin')
            <div class="sidebar-label">Registro y aprobaciones</div>
            ...
            <div class="sidebar-label">Operaciones de servicio</div>
        ...
        @if($rol === 'empresa')
            ...
            @if($empresaActiva)
                <a wire:navigate href="{{ route('empresa.solicitudes') }}" class="sidebar-link {{ request()->routeIs('empresa.solicitudes*') ? 'active' : '' }}">
        ...
        @if($rol === 'candidato')
            ...
            @if($candidatoAprobado)
                ...
                <a wire:navigate href="{{ route('candidato.postulaciones') }}" class="sidebar-link {{ request()->routeIs('candidato.postulaciones*') ? 'active' : '' }}">
        ...
        @if($rol === 'interno')
            ...
            <a wire:navigate href="{{ route('interno.tareas.index') }}" class="sidebar-link {{ request()->routeIs('interno.tareas*') ? 'active' : '' }}">
                <svg fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" d="M9 12.75 11.25 15 15 9.75M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z"/></svg>
                Mis tareas
            </a>
        @endif

        @if(in_array($rol, ['admin', 'empresa', 'interno'], true))
            <div class="sidebar-label">Soporte</div>
            <a wire:navigate href="{{ route('tickets.index') }}" class="sidebar-link {{ request()->routeIs('tickets.*') ? 'active' : '' }}">
        ...
        <a wire:navigate href="{{ route('chat.index') }}" class="sidebar-link {{ request()->routeIs('chat.*') ? 'active' : '' }}">
